...
func main() {
	...
	a := map[string]string{"aA": "X", "AB": "X"}
	b := map[string]string{"AB": "X", "aA": "X"}
	...
	if s1 == s2 {
		fmt.Println("Maps are equal", s1, s2)
	} else {
		fmt.Println("Maps are not equal", s1, s2)
	}
...
func exercise() {
	...
	*/
	...
	var m2 = map[int]string{1: "Logu", 2: "Sathya"}
	p("%#v\n", m2)
